...
const RenderSubCategory = function (subCategory, markdownRemark) {
	...
	return (
		<>
			<div className={"subcategory-title"} onClick={() => showFirstElement()} onKeyDown={() => showFirstElement()} role='button' tabIndex={0}>
				{subCategory.title}
			</div>
						...
						<div key={index} className={"row"}>
							<div className={"col-12"}>
								<div className={"article-title"}>
									<Link
										className={`article-link ${article.slug === markdownRemark.frontmatter.slug && "active"}`}
										to={`${article.slug}/`}
...
export default function TocGroup(props) {
	...
	return (
		<div key={props.key} className={`col-12 group ${isOpen ? "opened" : "closed"}`}>
			<div className={"group-title"} onClick={() => setIsOpen(!isOpen)} onKeyDown={() => setIsOpen(!isOpen)} role='button' tabIndex={0}>
				{group.category}
			...
			<div className={"group-items"}>
				{group.subcategories &&
					group.subcategories.map(function (subCategory, subCategoryIndex) {
						return <>{RenderSubCategory(subCategory, markdownRemark)}</>;
					})}
			</div>
		</div>
	);
